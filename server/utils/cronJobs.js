...
const generateMonthlyRepaymentDues = async () => {
  ...
  try {
    const activeLoans = await FundedLoan.find({ status: 'active', repaymentSchedule: { $exists: false } });
...
const checkOverduePayments = async () => {
  ...
  try {
    const overdueLoans = await FundedLoan.find({ status: 'active', nextRepaymentDate: { $lt: today } });
    let overdueCount = 0;
    for (const loan of overdueLoans) {
      const overdueInstallment = loan.repaymentSchedule?.find(i => i.status === 'pending' && new Date(i.dueDate) < today);
...
const sendPaymentReminders = async () => {
  ...
  try {
    const upcomingPayments = await FundedLoan.find({
      status: 'active',
      nextRepaymentDate: { $gte: today, $lte: threeDaysFromNow }
...
module.exports = { generateMonthlyRepaymentDues, checkOverduePayments, sendPaymentReminders };